...
async function doFootprintLatlonSearch(lat, lon) {
    const latLonApiQuery = 'https://data.cityofnewyork.us/api/geospatial/7w4b-tj9d?lat=' + lat + '&lon=' + lon + '&zoom=1';
    writeSearchLog('\r\n"Building Footprints" API query by LatLon ' + latLonApiQuery + '\r\n');
    let response = await fetch(latLonApiQuery);
    if (response.ok) {
        footprintJson = await response.json();
        if (footprintJson.length > 0) {
            writeSearchLog(JSON.stringify(footprintJson));
        }
    }
}
...
async function doFootprintBinSearch(bin) {
    ...
    */
    ...
    writeSearchLog('\r\n"Building Footprints" API query by BIN ' + footprintApiQuery + '\r\n');
